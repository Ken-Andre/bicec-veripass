...
async def trigger_ocr_extraction(
    ...
    task = demo_extract_cni.apply_async(
        args=[document_id, simulate_failure],
        queue="glm_ocr_jobs"
    )
...
async def trigger_liveness_check(selfie_id: str = "SELFIE_001"):
    ...
    """
    task = demo_verify_liveness.apply_async(
        args=[selfie_id],
        queue="glm_ocr_jobs"
    )
...
async def trigger_otp_sms(
    ...
    """
    task = demo_send_otp.apply_async(
        args=[phone, otp_code],
        queue="notifications"
    )
...
async def trigger_kyc_email(
    ...
    """
    task = demo_send_kyc_result.apply_async(
        args=[email, status],
        queue="notifications"
    )
...
async def trigger_batch_provision(user_count: int = 5):
    ...
    task = demo_batch_provision.apply_async(
        args=[user_ids],
        queue="notifications"
    )
