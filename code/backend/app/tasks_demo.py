...
def demo_extract_cni(document_id: str, simulate_failure: bool = False):
    ...
    time.sleep(2)
    print("📦 [OCR] Modèle GLM-OCR chargé")
    ...
    print(
        f"✅ [OCR] Extraction réussie CNI #{document_id} - {result['extracted_data']['nom']} {result['extracted_data']['prenom']}"
    )
    return result
...
def demo_verify_liveness(selfie_id: str):
    ...
    time.sleep(3)
    print("🧠 [LIVENESS] MiniFASNet en cours...")
    time.sleep(2)
